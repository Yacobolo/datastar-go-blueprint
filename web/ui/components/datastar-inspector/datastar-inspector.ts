...
export class DatastarInspector extends LitElement {
  ...
  private renderToggle() {
    return html`
      <button
        class="btn btn-primary btn-circle btn-sm fixed bottom-4 right-4 z-[99999] shadow-xl ${this.hasUnseenChanges ? 'animate-pulse' : ''}"
        @click=${this.toggle}
  ...
  private renderPanel(filteredSignals: SignalObject, filteredCount: number, hasFilter: boolean) {
    return html`
      <div class="fixed bottom-4 right-4 z-[99999] flex h-[32rem] w-96 max-w-[calc(100vw-2rem)] flex-col overflow-hidden rounded-box border border-base-300 bg-base-100 shadow-2xl">
  ...
  private renderContent(filteredSignals: SignalObject, hasFilter: boolean) {
    ...
    return html`
      <div class="flex-1 overflow-auto p-3">
        ${isEmpty
          ? html`<div class="flex h-full items-center justify-center text-sm text-base-content/60">
        ...
        <table class="table table-zebra table-xs">
          ...
          <tbody>
            ${flattenSignals(signals).map(([path, value]) => {
              ...
              return html`
                <tr class="${isChanged ? 'bg-warning/20' : ''}">
